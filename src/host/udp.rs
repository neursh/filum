...
use tokio::{ net::UdpSocket, select, sync::{ mpsc::{ self, Receiver, Sender }, Mutex } };
...
async fn make_new_socket(
    ...
    let proxied_client = match UdpSocket::bind(bridge_addr).await {
    ...
    if let Err(message) = proxied_client.connect(source_socket).await {
        println!("{}{}", client_port_log.bold().red(), message);
        packet_receiver.close();
        return;
    }
...
async fn server_cast(
    ...
    loop {
        ...
        tokio::select! {
            ...
            length = proxied_client.recv(&mut packet) => {
                match length {
...
async fn client_cast(
    ...
    loop {
        ...
        if let Err(message) = proxied_client.send(&received_packet).await {
